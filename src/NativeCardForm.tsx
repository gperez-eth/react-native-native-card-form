...
interface PrivateFieldEvent extends CardFieldState {
  field: CardField;
  brand: CardBrand;
}

interface PrivateNativeViewProps extends ViewProps {
  sessionId: string;
  field: CardField;
  disabled: boolean;
  placeholder: string;
  ...
  enteredAccessibilityValue: string;
  invalidAccessibilityValue: string;
  textColor: string;
  ...
  onStateChange(event: NativeSyntheticEvent<PrivateFieldEvent>): void;
}

const NativeModule = requireNativeModule<NativeCardSessionModule>('MackenrowNativeCard');
const PrivateNativeField =
  requireNativeViewManager<PrivateNativeViewProps>('MackenrowNativeCard');
const DEFAULT_TIMEOUT_MS = 30_000;
const EMPTY_FIELD: CardFieldState = { status: 'empty', focused: false, touched: false };
const INITIAL_STATE: NativeCardFormState = {
  complete: false,
  brand: 'unknown',
  fields: {
    number: EMPTY_FIELD,
...
interface FieldProps {
  ...
  props: NativeCardFormProps;
  onNativeChange(event: NativeSyntheticEvent<PrivateFieldEvent>): void;
  trailing?: React.ReactNode;
}

function Field({ field, sessionId, state, props, onNativeChange, trailing }: FieldProps) {
  ...
  const error =
    state.status === 'invalid'
      ...
      : state.touched && state.status !== 'valid' && state.status !== 'empty'
        ...
        : undefined;
  const inputText = StyleSheet.flatten(appearance?.inputStyle) as TextStyle | undefined;
  ...
  const fontWeight =
    typeof inputText?.fontWeight === 'string'
  ...
  return (
    <View style={[styles.fieldColumn, appearance?.fieldStyle]}>
      ...
      >
        <PrivateNativeField
          ...
          enteredAccessibilityValue={strings.enteredValueLabel}
          invalidAccessibilityValue={strings.invalidAccessibilityLabel}
          ...
          onStateChange={onNativeChange}
          placeholder={placeholder}
          placeholderColor={appearance?.placeholderColor ?? '#8A8A93'}
          cursorColor={appearance?.cursorColor ?? '#E74949'}
          sessionId={sessionId}
          style={[
            styles.nativeInput,
      ...
      {error ? (
        <Text
          ...
          style={[styles.error, appearance?.errorStyle]}
        >
          {error}
        </Text>
      ) : null}
...
function ValidMark() {
  return (
    <View accessible={false} pointerEvents="none" style={styles.validMark}>
 ...
 */
export const NativeCardForm = forwardRef<NativeCardFormRef, NativeCardFormProps>(
  function NativeCardForm(props, ref) {
    const {
      autoFocus = false,
      onChange,
      renderBrand,
      testID,
      tokenizationTimeoutMs = DEFAULT_TIMEOUT_MS,
    } = props;
    const reactId = useId();
    const sessionId = `native-card-form:${reactId}`;
    const disposed = useRef(false);
    const stateRef = useRef<NativeCardFormState>(INITIAL_STATE);
    const [state, setState] = useState<NativeCardFormState>(INITIAL_STATE);
    ...
    const updateField = useCallback(
      (event: NativeSyntheticEvent<PrivateFieldEvent>) => {
    ...
    useImperativeHandle(
      ...
      () => ({
        async tokenize() {
          if (disposed.current) throw asNativeCardFormError({ code: 'disposed' });
          try {
    ...
    const brandNode = useMemo(() => {
      ...
      return renderBrand
        ? React.createElement(renderBrand as React.ComponentType<typeof rendererProps>, rendererProps)
        : <DefaultBrand {...rendererProps} />;
    }, [brandLabel, renderBrand, state.brand]);

    return (
      <View
        style={[styles.form, props.appearance?.formStyle]}
        testID={testID}
        accessibilityRole="none"
      >
        <Field
          field="number"
          onNativeChange={updateField}
          props={props}
          sessionId={sessionId}
          state={state.fields.number}
          trailing={brandNode}
        />
